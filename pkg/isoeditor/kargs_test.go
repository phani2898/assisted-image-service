...
`
	...
	Describe("kargsConfigBoundariesFinder", func() {
		var (
			mockBoundariesFinderStart int64 = 1000
			mockBoundariesFinderSize  int64 = 5000
		)
		...
		It("returns error if file not in config", func() {
			kargsJSON := []byte(`{ "files": [] }`)
			_, _, err := kargsConfigBoundariesFinder("dummy.iso", "other.img", kargsJSON, mockBoundariesFinder)
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("not found in kargs config"))
		})

		It("returns error if invalid json", func() {
			_, _, err := kargsConfigBoundariesFinder("dummy.iso", "cdboot.img", []byte("invalid"), mockBoundariesFinder)
			Expect(err).To(HaveOccurred())
		})
	})
})
